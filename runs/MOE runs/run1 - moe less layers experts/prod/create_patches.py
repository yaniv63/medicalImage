...
def can_extract_patch(shape, zc, yc, xc, w):
    return (xc-w) >= 0 and (xc+w+1)<=(shape[2]-1) and (yc-w) >= 0 and (yc+w+1)<=(shape[1]-1)\
           and (zc-w) >= 0 and (zc+w+1)<=(shape[0]-1)


def extract_axial(vol,zc, yc, xc, w):
    x = np.arange(xc - w, xc + w +1, 1)
    y = np.arange(yc - w, yc + w +1, 1)
