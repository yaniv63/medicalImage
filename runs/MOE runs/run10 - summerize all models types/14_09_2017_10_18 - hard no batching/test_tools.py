...
def model_pred(weight_dir, input_q, output_q, args, unimodel=False):
    ...
    while True:
        ...
        if not use_stats_model:
            ...
            stats = zip(stats[0],stats[1],stats[2],stats[3])#,stats[4],stats[5],stats[6])
...
def load_model(weight_dir,args):
    ...
    #combined_model.load_weights(weight_dir + 'combined_weights_1_real_bak.h5'.format(args['test_person']),by_name=True)
    ...
    combined_model.load_weights(w_path_gate + 'gate_parameters_test1_soft_no_batching.h5', by_name=True)
...
def test_coefficients_model(w_path,args):
    ...
    model.load_weights(w_path_gate + 'gate_parameters_test1_soft_no_batching.h5', by_name=True)
