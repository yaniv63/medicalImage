...
m="att_one"
w_file = "model_test_1_fold_0_loss.h5"
...
def load_unimodel(weight_dir,args):
    ...
    model.load_weights(weight_dir+w_file)
...
def load_model(weight_dir,args):
    from multi_predictors_combined import n_experts_combined_model_gate_parameters,n_parameters_combined_model,\
        average_n_models_prediction,n_experts_combined_model_gate_attention,one_expert_attention
    from keras.optimizers import SGD
    ...
    if m=="moe":
        ...
        combined_model = average_n_models_prediction(N_mod=4,n=3)
    elif m=="conct":
        combined_model=n_parameters_combined_model(N_mod=4,n=3)
    elif m=="att":
        combined_model = n_experts_combined_model_gate_attention(N_mod=4,n=3)
    elif m=="att_one":
        combined_model = one_expert_attention(N_mod=4,n=3)
    
    optimizer = SGD(lr=0.01, nesterov=True)
    combined_model.compile(optimizer=optimizer, loss='binary_crossentropy', metrics=['accuracy', 'fmeasure'])
    combined_model.load_weights(weight_dir + w_file)#epoch_30-x-loss_0.203-x-fmeasure_0.9294.hdf5')#,by_name=True)

    combined_model.compile(optimizer=optimizer, loss='binary_crossentropy', metrics=['accuracy', 'fmeasure'])

    main_output_model = Model(input=combined_model.input,
                                     output=combined_model.outputs[0])

    return main_output_model
