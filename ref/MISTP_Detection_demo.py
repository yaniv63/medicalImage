...
def create_smodel():
    ...
    smodel.add(Flatten(input_shape=(48, 4, 4)))
    smodel.add(Dense(16))
    smodel.add(LeakyReLU())
    smodel.add(Dropout(0.25))
...
def create_slice_model():
    ...
    # output is a flat vector
    slice_model.add(Flatten(input_shape=(48,4,4)))
...
def create_full_model():
    ...
    for ch in range(img_channels):
        ...
        #graph.add_node(layer=s_model, name='s%d'%(ch), input='s%d_curr_model_node'%(ch))
        
        slice_model_node_list.append('s%d_curr_model_node'%(ch))
