...
def create_callbacks(name,fold):
    ...
    logger.debug("epoch {} loss {:.5f} acc {:.5f} fmeasure {:.5f} val_loss {:.5f} val_acc {:.5f} val_fmeasure{:.5f} ".
                 format(epoch, logs['loss'], logs['acc'], logs['fmeasure'], logs['val_loss'], logs['val_acc'],
                        logs['val_fmeasure'])))
    mycallbacks = [print_logs,save_weights]# stop_train_callback1, stop_train_callback2,save_weights]
    return mycallbacks
...
# ######## train model
logger.info("use adadelta lr 0.1")
...
for i,(train_index, val_index) in enumerate(kf.split(person_indices)):
    ...
    ad = adadelta(lr = 0.1)
    predictors[i].compile(optimizer=ad, loss='binary_crossentropy', metrics=['accuracy', 'fmeasure'])
